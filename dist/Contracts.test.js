...
describe('Contracts class', () => {
    ...
    describe('registerIdentity method', () => {
        ...
        const sendSignedTransactionSuccess = jest.fn((type, cb) => jest.fn(cb(transactionReceipt)));
        ...
        it('should call relevant functions and return ALREADY_REGISTERED for registered Id', async () => {
            ...
            const contracts = (await Promise.resolve().then(() => require('./Contracts'))).default;
            await expect(contracts.registerIdentity(REGISTERED_IDENTITY, IDENTITY_PRIVATE_KEY, WALLET_ADDRESS, WALLET_PRIVATE_KEY, configuration)).resolves.toEqual('ALREADY_REGISTERED');
            expect(isRegistered).toHaveBeenCalled();
        });
        it('should call relevant functions and return transaction receipt for unregistered Id', async () => {
            isRegisteredCall.mockImplementation(() => false);
            const contracts = (await Promise.resolve().then(() => require('./Contracts'))).default;
            await expect(contracts.registerIdentity(UNREGISTERED_IDENTITY, IDENTITY_PRIVATE_KEY, WALLET_ADDRESS, WALLET_PRIVATE_KEY, configuration)).resolves.toBe(transactionReceipt.transactionHash);
    ...
    describe('approveMission method', () => {
        ...
        it('should call relevant functions and return transaction receipt', async () => {
            const contracts = (await Promise.resolve().then(() => require('./Contracts'))).default;
            await expect(contracts.approveMission(REGISTERED_IDENTITY, WALLET_PRIVATE_KEY, configuration)).resolves.toBe(transactionReceipt);
    ...
    describe('watchContract method', () => {
        ...
        it('should call relevant functions and receive contract events', async () => {
            ...
            const spy = jest.fn();
            const contracts = (await Promise.resolve().then(() => require('./Contracts'))).default;
            const observable = contracts.watchContract(REGISTERED_IDENTITY, common_enums_1.ContractTypes.basicMission, configuration);
