...
export default class Bid<T extends BidParams, U extends MessageParams> {
    ...
    public async accept<V extends MissionParams>(missionParams: V, walletPrivateKey: string): Promise<Mission<V, U>> {
        // TODO: probably change to this._params.id or something, to send to Channel#6
        const bidderId = this._params.needTypeId; // Channel #2
    ...
    public async missions<V extends MissionParams>(missionParamsType: new (...all: any[]) => V): Promise<Observable<Mission<V, U>>> {
        // Channel#2 - after bug fix should change to Channel#6
