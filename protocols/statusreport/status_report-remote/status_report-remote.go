...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  void report_status(DAVUser vehicleID, VehicleState state)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := statusreport.NewStatusReportClientFactory(trans, protocolFactory)
  ...
  case "report_status":
    if flag.NArg() - 1 != 2 {
      fmt.Fprintln(os.Stderr, "ReportStatus requires 2 args")
      flag.Usage()
    }
    arg4 := flag.Arg(1)
    ...
    argvalue1 := vehicle.NewVehicleState()
    ...
    value1 := argvalue1
    fmt.Print(client.ReportStatus(value0, value1))
    fmt.Print("\n")
    break
  case "":
    Usage()
