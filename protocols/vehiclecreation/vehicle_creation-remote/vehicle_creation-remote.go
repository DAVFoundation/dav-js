...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  void create_vehicle(VehicleDetails vehicle)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := vehiclecreation.NewVehicleCreationClientFactory(trans, protocolFactory)
  ...
  case "create_vehicle":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "CreateVehicle requires 1 args")
      flag.Usage()
    }
    arg4 := flag.Arg(1)
    ...
    value0 := argvalue0
    fmt.Print(client.CreateVehicle(value0))
    fmt.Print("\n")
    break
  case "":
    Usage()
